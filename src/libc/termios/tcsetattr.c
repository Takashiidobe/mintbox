...
int tcsetattr(int fd, int action, const struct termios *stp) {
  ...
  flags = ((stp->c_cflag & CSTOPB) ? _TF_2STOP : _TF_1STOP) |
          ...
          ((stp->c_iflag & BRKINT) ? _TF_BRKINT : 0) | (stp->c_cflag & CSIZE);
  state = (stp->c_cflag & HUPCL) ? _TS_HPCL : 0;
  sg.sg_flags |=
      ((stp->c_cflag & PARENB) ? ((stp->c_cflag & PARODD) ? ODDP : EVENP) : 0);
  sg.sg_flags |=
      ((stp->c_lflag & (TOSTOP | NOFLSH | ECHOCTL | ECHO)) |
